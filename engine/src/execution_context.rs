...
impl<'e, U> ExecutionContext<'e, U> {
    ...
    /// Atomically update user_data and clear all field values.
    ///
    /// This is the recommended API for per-request detection:
    /// register lazy field methods once, then call `update()` for each request.
